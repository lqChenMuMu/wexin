...
import com.cl.wechat.base.advanced.model.WeixinMedia;
...
public class MediaUtil extends CommonUtil {
	...
	public static WeixinMedia uploadMedia(String accessToken,
		...
		try {
			...
			weixinMedia.setType(jsonObject.getStr("type"));
			// type等于thumb时的返回结果和其他类型不一样
			if ("thumb".equals(type)) {
				weixinMedia.setMediaId(jsonObject.getStr("thumb_media_id"));
			}else {
				weixinMedia.setMediaId(jsonObject.getStr("media_id"));

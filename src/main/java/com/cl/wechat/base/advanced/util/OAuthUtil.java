...
import com.cl.wechat.base.advanced.model.SNSUserInfo;
import com.cl.wechat.base.advanced.model.WeixinOauth2Token;
	...
	public static WeixinOauth2Token getOauth2AccessToken(String appId,
		...
		if (null != jsonObject) {
			try {
				...
				System.err.println(jsonObject.getStr("access_token"));
	...
	public static SNSUserInfo getSNSUserInfo(String accessToken, String openId) {
		...
		if (null!=jsonObject) {
			try {
				sui=new SNSUserInfo();
				// 用户标识
				sui.setOpenId(jsonObject.getStr("openid"));
				// 昵称
				sui.setNickname(jsonObject.getStr("nickname"));
				// 性别(1是男性，2是女鞋，3是未知)
				sui.setSex(jsonObject.getInt("sex"));
				// 用户所在国家
				sui.setCountry(jsonObject.getStr("country"));
				// 用户所在省份
				sui.setProvince(jsonObject.getStr("province"));
				// 用户所在城市
				sui.setCity(jsonObject.getStr("city"));
				// 用户头像
				sui.setHeadImgUrl(jsonObject.getStr("headimgurl"));
				// 用户特权信息
				sui.setPrivilegeList(JSONUtil.toList(jsonObject.getJSONArray("privilege"),String.class));
			} catch (Exception e) {
				sui=null;
				int errorCode=jsonObject.getInt("errcode");
				String errorMsg=jsonObject.getStr("errmsg");
